...
**裁剪列表必须同时挪动分页游标。** `MAX_ITEMS = 400` 是内存上限，但 `oldest` 是「下一页取它之前的
内容」的游标——翻过 400 条历史后再来一条新消息，队首被丢掉而游标还指着它，`loadOlder()` 就会取
「一条已经不在列表里的消息之前」的那一页，直接拼到幸存者前面，**中间那段历史永久消失**（`hello`
只带最新 50 条，滚回去也找不回来）。所以 `appended()` 在裁剪后重设 `oldest = kept.first().id` 并把
`hasMore` 置真——即使之前已经加载到会话开头。
